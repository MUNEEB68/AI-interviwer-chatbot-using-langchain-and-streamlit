...
def main():
    ...
    handle_cv_upload(Assistant,step_placeholder)
    #step 2: Display all the available jobs
    if st.session_state.cv_uploaded:
        step_placeholder.empty()
        Handle_jobs_roleselection(Assistant)
    if st.session_state.job_selected:
        step_placeholder.empty()
        time.sleep(10)
        Check_eligibility(Assistant,step_placeholder)
    ...
    if st.session_state.final_evaluation_done:
     ...
     your_password = #enter your app password for the gmai;
